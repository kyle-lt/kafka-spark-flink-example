...
public class KafkaFlinkConsumerExample {
	...
	public static void main(final String... args) {
		...
		//DataStream<String> messageStream = env.addSource(new FlinkKafkaConsumer010<>(Commons.EXAMPLE_KAFKA_TOPIC, new MessageDeserializer(), props));
		
		DataStream<String> messageStream = env
				.addSource(new FlinkKafkaConsumer<>(Commons.EXAMPLE_KAFKA_TOPIC, new MessageDeserializer(), props));
		...
		*/
		...
		messageStream.print();
		//messageStream.rebalance().map(new MessageProcessor()).print();
		
		/*
		// Split up the lines in pairs (2-tuples) containing: (word,1)
		messageStream.flatMap(new Tokenizer()) // group by the tuple field "0" andsum up tuple field "1" 
		.keyBy(0) .sum(1) .print();
		*/
